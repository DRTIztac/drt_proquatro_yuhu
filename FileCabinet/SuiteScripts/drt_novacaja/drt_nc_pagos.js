 ...
 */
 define(['N/search', 'N/render', 'N/file', 'N/record', 'N/runtime', 'N/config', 'N/url', 'N/xml', 'N/format'],
  ...
  function(search,render,file,record) {
      ...
      try{
        ...
        if(!existePago){
          ...
          if(jsonPayment.excedente > 0 )
          {
           ...
           var deposit= createCustomerDeposit(param_body);
          log.debug({title:"createCustomerDeposit ",details: JSON.stringify(deposit)});
    ...
    function createCustomerDeposit(param_body){
      const param_record= 'customerdeposit';
        var respuesta = {
          success: false,
          data: '',
          error: {}
